document.addEventListener('DOMContentLoaded', function () {
    ...
    counters.forEach(counter => {
        ...
        let current = 0;
        const increment = target / (duration / 16); // 16ms ចន្លោះពេល (ប្រហែល 60fps)

        function updateCounter() {
            if (current < target) {
                current += increment;
                counter.textContent = Math.ceil(current); // បង្រួមឡើងទៅលេខគត់
                requestAnimationFrame(updateCounter);
            } else {
                counter.textContent = target; // ធានាថាលេខចុងក្រោយគឺត្រឹមត្រូវ
            }
        }
...
document.querySelectorAll(".card-categories").forEach(c => {
