...
tickets.forEach((ticket, i) => {
  ...
  document.getElementById("ticketType").innerHTML += `
        <option data-remain="${ticket.ticket_opacity - ticket.ticket_bought}" ${
    i == 0 && "selected"
  } value='${ticket.id}' ${isDisabled}>${ticket.type} - $${ticket.price.toFixed(
    2
  )}${" " + soldOutText}</option>
    `;
  if (i == 0) {
    document.getElementById("remain-ticket").innerText =
      ticket.ticket_opacity - ticket.ticket_bought;
  }
});
...
<div class="event-card shadow-light-sm">
                        ...
                        <div class="event-details">
                            <div
                                ...
                                <h3
                                    class="event-title" style="color: #333;">${
                                      events.eng_name
                                    }</h3>
                            ...
                            <div class="event-meta">
                                <div class="event-meta-item">
                                    ...
                                    <span>${moment(events.started_date).format(
                                      "ll"
                                    )}</span>
                                </div>
                                <div class="event-meta-item">
                                    ...
                                    <span>${
                                      moment(events.start_time, "HH:mm").format(
                                        "LT"
                                      ) +
                                      " - " +
                                      moment(events.end_time, "HH:mm").format(
                                        "LT"
                                      )
                                    }</span>
                                </div>
...
document.getElementById("submitButton").addEventListener("click", async (e) => {
  ...
  if (!validateResult.valid) {
    document.querySelector(".upload-box").style.border =
      "2px dashed var(--bs-danger) !important";
    document.getElementById("invalid-qr-msg").classList.remove("d-none");
    document.querySelector("#invalid-qr-msg span").innerText =
      validateResult.message;
    return;
  }

  document.querySelector(".upload-box").style.border =
    "2px dashed var(--c-brand) !important";
  ...
  try {
    ...
    showToast(
      true,
      isEnglish
        ? "Purchase ticket has submitted. Please wait for confirmation from organizer."
        : "ការទិញសំបុត្រត្រូវបានដាក់ស្នើ។ សូមរង់ចាំការបញ្ជាក់ពីអ្នករៀបចំ"
    );
    ...
    btnCloseLoading(
      "submitButton",
      isEnglish ? "Submit Purchase" : "ដាក់ស្នើការទិញ"
    );
    // document.getElementById("submitButton").disabled = true
  }
});
...
function calculateTotal() {
  ...
  if (match) {
    ...
    const subtotal =
      parseFloat(document.getElementById("ticketQuantity").value) * price;
