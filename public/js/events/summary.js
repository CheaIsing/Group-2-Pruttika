...
async function getSummary() {
  ...
  const formattedDate = `${moment(json.started_date).format(
    "MMM D, YYYY"
  )} - ${moment(json.ended_date).format("MMM D, YYYY")}, ${moment(
    json.start_time,
    "HH:mm"
  ).format("LT")} - ${moment(json.end_time, "HH:mm").format("LT")}`;
  console.log(json);
  ...
  document.getElementById("event-type").innerText =
    json.event_type == 1 ? "Online Event" : "In Person Event";
  ...
  document.getElementById("participated").innerText =
    (json.total_checkin ? json.total_checkin : "0") + " attendant";
  document.getElementById("not-participated").innerText =
    json.total_approved_registrations - json.total_checkin + " attendant";
  ...
  <tr
    ...
    <td
        class="text-end">-</td>
    <td class="text-end">
        ${json.ticket.reduce((sum, ticket) => sum + ticket.ticket_bought, 0)}
    </td>
    <td
