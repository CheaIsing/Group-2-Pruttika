      ...
      followers.forEach((f) => {
        ...
        <div class="list-group bg-white p-3 rounded-3 shadow-light-sm" >
                        <a class="d-flex align-items-center justify-content-between">
                            <div class="d-flex align-items-center flex-grow-1 h-hover" onclick="showProfile(${
                              f.id
                            })" role="button"> 
                                <img src="/uploads/${
                                  f.avatar ? f.avatar : "default.jpg"
                            ...
                            <button class="btn btn-outline-brand bg-transparent px-3 fw-medium" style="height: auto !important;" onclick="removeFollowByOther(${
                              f.id
                            }, this)">${isEnglish ? "Remove" : "ដកចេញ"}</button>
                        </a>
                    </div>`;
      });
      ...
      document.getElementById(
        "followers"
              ...
              <h4 class="text-center text-brand mt-2">${
                isEnglish
                  ? "No Follower to Display"
                  : "គ្មានអ្នកតាមដានដើម្បីបង្ហាញ"
              }</h4>
            </div>`;
    }
      ...
      following.forEach((f) => {
        ...
        <div class="list-group bg-white p-3 rounded-3 shadow-light-sm" >
                        <div class="d-flex align-items-center justify-content-between" >
                            ...
                            <button class="btn btn-outline-brand bg-transparent px-3 fw-medium" style="height: auto !important;" onclick="toggleFollow(${
                              f.id
                            }, this)">${getText("unfollow")}</button>
                        </div>
                    </div>`;
      });
...
async function removeFollowByOther(id, list) {
  try {
    ...
    showToast(
      true,
      isEnglish ? "Remove follow successfully." : "លុបការតាមដានដោយជោគជ័យ"
    );
