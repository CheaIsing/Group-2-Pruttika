...
const postRequestTicket = async (req, res) => {
  const { ticket_type_id, quantity, event_id } = req.body;
  const buyer_id = req.user.id;

  try {
    //check if ticket is online or offline
    const eventDetails = await executeQuery(
      `SELECT event_type FROM tbl_event WHERE id = ?`,
      [event_id]
    );
    ...
    if (eventDetails[0].event_type == 1) {
      ...
      return sendResponse(
        ...
        200,
        true,
        `Request for online event successfully created.`
      );
      ...
      if (
        ...
        return sendResponse(
          ...
          400,
          false,
          "Ticket type ID is required for offline events."
        );
      }
      ...
      if (!checkTicket) {
        return sendResponse(
          ...
          400,
          false,
          `Event Ticket Type id ${ticketTypeId} is not found`
        );
      }
      if (checkTicket.ticket_bought + qty > checkTicket.ticket_opacity) {
        return sendResponse(
          ...
          400,
          false,
          `This tickets are sold out or the number of tickets you requested exceeds the remaining availability.`
        );
      }
      ...
      const result = await executeQuery(sqlInsertReq, [
        buyer_id,
        ticketTypeId,
        qty,
        amount,
...
const updateTransactionFile = async (req, res) => {
  ...
  const { error } = fileSchema.validate({
    transaction_file: req.files ? req.files.transaction_file : undefined,
  });
...
const deleteTransactionFile = async (req, res) => {
  ...
  try {
    ...
    sendResponse(
      ...
      200,
      true,
      "Transaction File of Ticket Request is delete successfully"
    );
  } catch (error) {
    handleResponseError(res, error);
  }
...
const putApproveTicket = async (req, res) => {
  ...
  try {
    ...
    const dbResult = await executeQuery(sqlGetTransaction, [
      ticketReq_id,
      user_id,
    ]);
    ...
    if (dbResult.length === 0) {
      return sendResponse(
        ...
        404,
        false,
        "Ticket Request ID not found or You do not have permission to Approved this ticket"
      );
    }
    ...
    const checkApprove = await executeQuery(
      `select * from tbl_transaction where id=? AND status=?`,
      [ticketReq_id, 2]
    );
    ...
    if (checkApprove.length > 0) {
      return sendResponse(
        ...
        400,
        false,
        "This Request is already have been Approved!"
      );
    }
    ...
    if (event_type == 2) {
      ...
      for (let i = 0; i < ticket_qty; i++) {
        promises.push(
          (async () => {
            try {
              const resultInsert = await executeQuery(sqlInsertTicket, [
                ticketReq_id,
                ticket_event_id,
              ]);
              ...
              await executeQuery(
                `UPDATE tbl_ticket SET qr_code = ?, qr_code_img = ? WHERE id = ?`,
                [token, qr_code_img, ticket_id]
              );
            } catch (error) {
              console.log(error);
              handleResponseError(res, error);
      ...
      await executeQuery(
        `UPDATE tbl_ticketevent_type SET ticket_bought=? WHERE id=?`,
        [ticket_bought + ticket_qty, ticket_event_id]
      );
    ...
    await executeQuery(`UPDATE tbl_transaction SET status = ? WHERE id = ?`, [
      2,
      ticketReq_id,
    ]);
    ...
    sendResponse(
      ...
      200,
      true,
      "Ticket Request has been approved successfully"
    );
  } catch (error) {
...
const putRejectTicket = async (req, res) => {
  ...
  if (!rejected_reason || rejected_reason.trim().length === 0) {
    return sendResponse(
      ...
      404,
      false,
      " You must need to input the reason why the ticket got rejected!"
    );
  }
  try {
    const sqlGetTransaction = `
    ...
    if (dbResult.length === 0) {
      return sendResponse(
        ...
        404,
        false,
        "Ticket Request ID not found or You do not have permission to Reject this ticket"
      );
    }

    const { id, status, event_id, buyer_id, eng_name } = dbResult[0];
    ...
    if (dbResult[0].status === 3) {
      return sendResponse(
        ...
        400,
        false,
        "This Ticket Request has already been rejected."
      );
      ...
      return sendResponse(
        ...
        400,
        false,
        "Cannot reject an approved Ticket Request."
      );
    }
    ...
    await executeQuery(sqlUpdateTransaction, [
      3,
      rejected_reason,
      ticketReq_id,
    ]);
    ...
    sendResponse(
      ...
      200,
      true,
      `Ticket Request ID ${ticketReq_id} has been rejected successfully`
    );
  } catch (error) {
    console.log(error);
    handleResponseError(res, error);
...
const getAllRequestTicket = async (req, res) => {
  ...
  try {
    ...
    sendResponse1(
      ...
      200,
      true,
      "Get all user request ticket successfully",
      data.rows,
      data.paginate
    );
  } catch (error) {
