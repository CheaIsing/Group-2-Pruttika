...
const postCreateEvent = async (req, res) => {
  ...
  if (agendaErrors.length > 0) {
    return res
      .status(400)
      .json({ message: "Validation errors", errors: agendaErrors });
  }
  ...
  try {
    ...
    for (const ticket of event_tickets) {
      const sqlInsertTicket = `
            ...
            `;
      await executeQuery(sqlInsertTicket, [
        eventId,
        ticket.type,
        ticket.price,
        ticket.ticket_opacity,
      ]);
    }
    ...
    for (const agd of agenda) {
      ...
      const sqlInsertAgenda = `
            ...
            `;
      await executeQuery(sqlInsertAgenda, [
        eventId,
        agd.title,
        agd.description,
        agd.start_time,
  ...
  try {
    ...
    for (const ticket of event_tickets) {
      if (!ticket.id) {
        ...
        const checkResult = await executeQuery(sqlCheckTicket, [
          ticket.id,
          event_id,
        ]);
        if (checkResult.length > 0) {
          ...
          return res
            .status(404)
            .json({ error: "Ticket not found", ticketId: ticket.id });
        }
      }
    }
    ...
    for (const agd of agenda) {
      if (!agd.id) {
        ...
        if (checkResult.length > 0) {
          ...
          return res
            .status(404)
            .json({ error: "Agenda not found", AgendaId: agd.id });
        }
      }
    }
    ...
    const sqlGetBuyer = `SELECT DISTINCT buyer_id, ticket_event_id from tbl_transaction WHERE event_id=? AND status=2`;
    const getBuyer = await executeQuery(sqlGetBuyer, [event_id]);

    // Keep track of notified buyers
    ...
    for (let i = 0; i < getBuyer.length; i++) {
      ...
      if (!notifiedBuyers.has(dataBuyer.buyer_id)) {
        ...
        notifiedBuyers.add(dataBuyer.buyer_id);
      }
...
const updateEThumbnail = async (req, res) => {
  ...
  const { error } = fileSchema.validate({
    thumbnail: req.files ? req.files.thumbnail : undefined,
  });
  if (handleValidateError(error, res)) return;
...
const deleteEAgenda = async (req, res) => {
  ...
  try {
    ...
    if (result.length === 0) {
      //check if invalid agenda
      return sendResponse(
        res,
        400,
        false,
        `Agenda id ${agenda_id} is not found`
      );
    }
    //if exist, delete
    ...
    sendResponse(
      ...
      200,
      true,
      `Event Agenda id "${agenda_id}" is delete successfully`
    );
  } catch (error) {
    handleResponseError(res, error);
  }
...
const deleteETickType = async (req, res) => {
  ...
  try {
    ...
    if (result.length === 0) {
      //check if invalid ticket type to delete
      return sendResponse(
        res,
        400,
        false,
        `Ticket Type id ${ticketType_id} is not found to delete`
      );
    }
    ...
    sendResponse(
      ...
      200,
      true,
      `Ticket Type id "${ticketType_id}" is delete successfully`
    );
  } catch (error) {
    handleResponseError(res, error);
  }
...
const uploadEQr = async (req, res) => {
  ...
  const { error } = fileSchema.validate({
    qr_img: req.files ? req.files.qr_img : undefined,
  });
  if (handleValidateError(error, res)) return;
  ...
  try {
    ...
    if (status == 2) {
      return sendResponse(
        ...
        400,
        false,
        "This ticket token is already check-in!"
      );
    }
...
const summaryData = async (req, res) => {
  ...
  try {
    ...
    sendResponse(
      ...
      200,
      true,
      `Get summary data of event ID ${id} Successfully`,
      data
    );
  } catch (error) {
    handleResponseError(res, error);
...
const getAllCheckInTicket = async (req, res) => {
  ...
  try {
    ...
    sendResponse(
      ...
      200,
      true,
      "Get all check-in ticket of event successfully",
      data
    );
  } catch (error) {
    handleResponseError(res, error);
