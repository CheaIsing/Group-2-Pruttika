...
FROM tbl_event
WHERE event_type = 1
AND DATE_FORMAT(CONCAT(started_date, ' ', start_time), '%Y-%m-%d %H:%i') = DATE_FORMAT(DATE_ADD("${moment(
      new Date()
    ).format("YYYY-MM-DD HH:mm:ss")}", INTERVAL 10 MINUTE), '%Y-%m-%d %H:%i')
    `;
    ...
    if (events && events.length > 0) {
      for (const event of events) {
        ...
        console.log(
          `Reminder scheduled for event ${eventId} to be sent immediately.`
        );
      }
    } else {
